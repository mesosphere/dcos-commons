...
/**
 * Default implementation of a {@link TaskEnvRouter} which handles forwarding environment variables
 * from two sources:
 * <ul>
 * <li>
 * Scheduler's system environment: searches for environment variables that start with
 * {@code "TASKCFG_<TASKTYPE>_"} and/or {@code "TASKCFG_ALL_"}, and returns appropriate Maps for
 * task types based on that environment.
 * </li>
 * <li>
 * Manually-provided values: The service developer may provide custom values which will then be
 * forwarded to tasks as needed.
 * </li>
 * </ul>
 */
public class TaskEnvRouter {
  ...
  /**
   * Adds a custom environment value to be included in all pods. This may be overridden by the
   * Scheduler environment via {@code TASKCFG_ALL_*} or {@code TASKCFG_<POD_TYPE>_*} envvars, and
   * by any per-pod types added via {@link #setPodEnv(String, String, String)}.
   */
  ...
  /**
   * Adds a custom environment variable to be only included in the specified pod type. This may be
   * overridden by the Scheduler environment via {@code TASKCFG_ALL_*} or
   * {@code TASKCFG_<POD_TYPE>_*} envvars. This takes precedence over values specified via
   * {@link #setAllPodsEnv(String, String)} for pods with this type.
   */
  public TaskEnvRouter setPodEnv(String podType, String key, String value) {
    sdkPodConfigs
        .computeIfAbsent(podType.toLowerCase(), k -> new TreeMap<>())
        .put(key, value);
    return this;
  }
   ...
   * <p>
   * For example, given a task with type "index", this would match scheduler environment variables
   * starting with "TASKCFG_ALL_" and "TASKCFG_INDEX_" (with those prefixes removed), as well as
   * any appropriate envvars which were passed in via the Java API (with no modification). The
   * prior takes priority over the latter.
   *
   * @param podType the pod type to be matched against
   * @return any environment variables to include in tasks of type {@code podType}, or an empty map
   * otherwise
   */
  public Map<String, String> getConfig(String podType) {

    // Third priority: API settings against "all":
    Map<String, String> matchingConfig = new TreeMap<>(sdkGlobalConfig);
