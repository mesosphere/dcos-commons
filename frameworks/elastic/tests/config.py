...
def check_kibana_adminrouter_integration(path):
    ...
    return shakedown.wait_for(fun, timeout_seconds=KIBANA_DEFAULT_TIMEOUT, noisy=True)
...
def check_elasticsearch_index_health(index_name, color, service_name=PACKAGE_NAME):
    ...
    return shakedown.wait_for(fun, timeout_seconds=DEFAULT_TIMEOUT)
...
def wait_for_expected_nodes_to_exist(service_name=PACKAGE_NAME):
    ...
    return shakedown.wait_for(expected_nodes, timeout_seconds=DEFAULT_TIMEOUT)
