...
class TLSArtifactsGenerator {
   ...
   */
  Map<TLSArtifact, String> generate(
      CertificateNamesGenerator certificateNamesGenerator)
      throws Exception
  {
    ...
    List<X509Certificate> certificateChain =
        new ArrayList<>(caClient.chainWithRootCert(certificate));
    ...
    if (certificateChain.size() > 1) {
      endEntityCertificateWithChain
          .addAll(certificateChain.subList(0, certificateChain.size() - 1));
    }
    ...
    values.put(
        TLSArtifact.CA_CERTIFICATE,
        PEMUtils.toPEM(certificateChain.get(certificateChain.size() - 1))
    );
