...
1. Navigate into the correct framework for your needs. The scheduler runs under `marathon` with a task name matching the service name (default `cassandra`). Service nodes run under a framework whose name matches the service name (default `cassandra`).
