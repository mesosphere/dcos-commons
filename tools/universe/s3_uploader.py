...
class S3Uploader(object):
    ...
    def _renew_credentials(self):
        ...
        if (
            subprocess.run(
                "which maws".split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            ).returncode
            != 0
        ):
        ...
        log.info(
            "Upload attempt failed, renewing credentials for AWS profile {}".format(aws_profile)
        )
    ...
    def _get_aws_profile(self):
        ...
        try:
            ...
            raise Exception(
                "Unable to renew credentials: No AWS_PROFILE and credentials file is unparseable: {}".format(
                    creds_path
                )
            )
        ...
        if len(profile_names) == 0:
            raise Exception(
                "Unable to renew credentials: No AWS_PROFILE and no profiles found in credentials file: {}".format(
                    creds_path
                )
            )
            ...
            raise Exception(
                "Unable to renew credentials: No AWS_PROFILE and multiple profiles found in credentials file: {}".format(
                    creds_path
                )
            )
