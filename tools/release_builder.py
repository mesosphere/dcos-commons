...
###""".format(
    ...
    def _fetch_stub_universe(self):
        ...
        if not stub_universe_extension == ".json":
            raise Exception(
                "Expected .json extension for stub universe: {}".format(self._stub_universe_url)
            )
    ...
    def _unpack_stub_universe(self, stub_universe_json, scratchdir):
        ...
        if "packages" not in stub_universe_json:
            raise Exception(
                'Expected "packages" key in root of stub universe JSON: {}'.format(
                    self._stub_universe_url
                )
            )
        if len(stub_universe_json["packages"]) != 1:
            raise Exception(
                'Expected single "packages" entry in stub universe JSON: {}'.format(
                    self._stub_universe_url
                )
            )
    ...
    def _update_resource_json(self, package_json):
        ...
        log.info(
            "Replacing artifact prefix {} with {}".format(
    ...
    def _copy_artifacts_s3(self, scratchdir, original_artifact_urls):
        ...
        if ret == 0:
            if self._force_upload:
                log.info(
                    "Destination {} exists but force upload is configured, "
                    "proceeding...".format(self._uploader.get_s3_directory())
                )
            else:
                raise Exception(
                    "Release artifact destination already exists. "
                    "Refusing to continue until destination has been manually removed:\n"
                    "Do this:\n"
                    "    aws s3 rm --dryrun --recursive {}".format(
                        self._uploader.get_s3_directory()
                    )
                )
        elif ret > 256:
            raise Exception(
                "Failed to check artifact destination presence (code {}). "
                "Bad AWS credentials? Exiting early.".format(ret)
            )
        else:
            log.info(
                "Destination {} doesnt exist, proceeding...".format(
                    self._uploader.get_s3_directory()
                )
            )
    ...
    def move_package(self):
        ...
        return universe_converter_url_prefix + os.path.join(
    ...
    def release_package(self, commit_desc=""):
        ...
        try:
            ...
            log.error(
                ...
                "which must be manually deleted before retrying.".format(
                    self._uploader.get_s3_directory()
                )
            )
            raise
...
def main(argv):
    ...
    if operation == "release":
        ...
        if response.status < 200 or response.status >= 300:
            ...
            log.error(
                "You will need to manually create the PR against the branch that was pushed above."
            )
            return -1
        log.info("---")
