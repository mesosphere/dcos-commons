...
public class ConfigStore<T extends Configuration> implements ConfigTargetStore {
   ...
   */
  public void setConfigurationFactory(ConfigurationFactory<T> factoryInternal) {
    this.factory = factoryInternal;
  }
  ...
  public void clear(UUID id) throws ConfigStoreException {
    ...
    try {
      ...
      if (e.getReason() == Reason.NOT_FOUND) {
        ...
        logger.warn(
            "Requested configuration '{}' to be deleted does not exist at path '{}'", id, path);
        return;
      } else {
        throw new ConfigStoreException(e, String.format(
