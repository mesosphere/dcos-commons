...
/**
 * Validates that pods do not move from a virtual network that does not use port mapping (and thus
 * ignores the ports as Mesos resources) to a network (virtual or Host) that does use the ports
 * on the host machine.
 */
public class PodSpecsCannotChangeNetworkRegime implements ConfigValidator<ServiceSpec> {
  ...
  private static boolean podSpecUsesHostPorts(PodSpec podSpec) {
    // using HOST network, uses ports
    if (podSpec.getNetworks().size() == 0) {
      return true;
    }
      ...
      ServiceSpec newConfig)
  {
    ...
    try {
      ...
      return Collections.singletonList(
          ConfigValidationError.valueError(
              ...
              "null",
              "Duplicate pod types detected."
          ));
    }

    // Check the PodSpecs to make sure none of them make a transition from a state where they
    // use host ports to one where they don't (or vice versa).
    ...
    for (Map.Entry<String, PodSpec> kv : newPods.entrySet()) {
      ...
      // first we check that the new pod's networks (if present) do not erroneously use
      // port mapping
      List<String> offendingNetworkNames = getNetworksWithInvalidPortMappings(newPod);
      if (offendingNetworkNames.size() > 0) {
        String errorMessage = String.format(
            "New config has pod %s that indicates port mapping for virtual network(s) %s," +
                " that do not support port mapping.",
            newPod.getType(),
      ...
      // now we check that the none of the new pods move from not using host ports to using
      // them (or vice versa)
      ...
      if (oldPod != null && podSpecUsesHostPorts(oldPod) != podSpecUsesHostPorts(newPod)) {
        errors.add(ConfigValidationError.transitionError(
            ...
            newPod.getNetworks().toString(),
            String.format("New config has pod %s " +
                    "moving networks from %s to %s, changing its host " +
                    "ports requirements from %s to %s, not allowed.",
                newPod.getType(),
                oldPod.getNetworks().toString(),
                newPod.getNetworks().toString(),
                podSpecUsesHostPorts(oldPod),
                podSpecUsesHostPorts(newPod)
            )
        ));
      }
    }
