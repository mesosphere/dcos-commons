...
def test_state_properties_get():
    ...
    assert jsonobj[0] == "hello-0-server:taskStatus"
    assert jsonobj[1] == "hello-1-server:taskStatus"
    assert jsonobj[2] == "last-completed-update-type"
    assert jsonobj[3] == "suppressed"
    assert jsonobj[4] == "world-0-server:taskStatus"
    assert jsonobj[5] == "world-1-server:taskStatus"
