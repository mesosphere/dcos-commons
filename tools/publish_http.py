...
class HTTPPublisher(object):
    ...
    def build(self, http_url_root):
        ...
        logger.info("---")
        logger.info(
            "Copying {} artifacts into {}:".format(len(self._artifact_paths), self._http_dir)
        )
...
""".format(
    ...
    def add_repo_to_cli(self, repo_url):
        ...
        logger.info(
            "Checking for duplicate repositories: name={}, url={}".format(repo_name, repo_url)
        )
        cur_universes = subprocess.check_output("dcos package repo list --json".split()).decode(
            "utf-8"
        )
        for repo in json.loads(cur_universes)["repositories"]:
            ...
            if repo["name"] == repo_name or repo["uri"] == repo_url:
                logger.info(
                    "Removing duplicate repository: {} {}".format(repo["name"], repo["uri"])
                )
