...
public class MultiArtifactResource {
  ...
  private static final String RUN_ARTIFACT_URI_FORMAT =
      "http://%s/v1/service/%s/artifacts/template/%s/%s/%s/%s";
  ...
  {
    String hostnameAndPort = EndpointUtils.toSchedulerAutoIpEndpoint(
        frameworkName,
        schedulerConfig);
