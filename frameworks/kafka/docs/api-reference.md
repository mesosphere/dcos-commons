...
```bssh
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/endpoints/broker"
{
...
```bash
$ curl -H "Authorization: token=" "/service/kafka/v1/pod"
[
  "kafka-0",
  "kafka-1",
  "kafka-2"
...
```bash
$ curl -X PUT -H "Authorization: token=" "/service/kafka/v1/pod/kafka-1/restart"
[
  "pod": "kafka-1",
  "tasks": ["kafka-1-broker"]
]
...
```bash
$ curl -X PUT -H "Authorization: token=" "/service/kafka/v1/pod/kafka-1/replace"
[
  "pod": "kafka-1",
  "tasks": ["kafka-1-broker"]
]
...
```bash
$ curl -H "Authorization: token=" "/service/kafka/v1/topics"
[
  "topic1",
  "topic0"
]
...
```bash
$ curl -X POST -H "Authorization: token=" "<dcos_url>/service/kafka/v1/topics/topic1"
{
  "partitions": [
  {
    "0": {
...
```bash
$ curl -X POST -H "Authorization: token=" "/service/kafka/v1/topics/topic1?partitions=3&replication=3"
{
  "message": "Output: Created topic \"topic1\"\n"
}
```
...
```bash
$ curl -H "Authorization: token=" "/service/kafka/v1/topics/topic1/offsets?time=-1"
[
  {
    "2": "334"
  },
...
```bash
$ curl -X PUT -H "Authorization: token=" "<dcos_url>/service/kafka/v1/topics/topic1?operation=partitions&partitions=2"
{
  "message": "Output: WARNING: If partitions are increased for a topic that has a key, the partition logic or ordering of the messages will be affectednAdding partitions succeeded!n"
}
```
...
```bash
$ curl -X PUT -H "Authorization: token=" "<dcos_url>/service/kafka/v1/topics/topic1?operation=producer-test&messages=10"
{
  "message": "10 records sent, 70.422535 records/sec (0.07 MB/sec), 24.20 ms avg latency, 133.00 ms max latency, 13 ms 50th, 133 ms 95th, 133 ms 99th, 133 ms 99.9th.n"
}
```
...
```bash
$ curl -X DELETE -H "Authorization: token=" "<dcos_url>/service/kafka/v1/topics/topic1"
{
  "message": "Topic topic1 is marked for deletion.nNote: This will have no impact if delete.topic.enable is not set to true.n"
}
```
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/topics/under_replicated_partitions"
{
  "message": ""
}
```
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/topics/unavailable_partitions"
{
  "message": ""
}
```
...
```bash
 -H "Authorization: token=" "<dcos_url>/service/kafka/v1/state/properties/suppressed"
```
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/configurations"
[
  "319ebe89-42e2-40e2-9169-8568e2421023",
  "294235f2-8504-4194-b43d-664443f2132b"
]
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/configurations/319ebe89-42e2-40e2-9169-8568e2421023"
```
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/configurations/target"
```
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/plan/deploy"
{
  "phases": [
    {
      "id": "1915bcad-1235-400f-8406-4ac7555a7d34",
...
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/plan/deploy/continue"
```
...
$ curl -H "Authorization: token=" "<dcos_url>/service/kafka/v1/plan/deploy/interrupt"
```
      ...
      "task-specs": [
        {
          ...
          "command-spec": {
            "value": "mv -v *statsd*.jar /kafka_2.11-0.10.2.1/libs/ && ./bootstrap -resolve=false && exec /kafka_2.11-0.10.2.1/bin/kafka-server-start.sh /kafka_2.11-0.10.2.1/config/server.properties\n",
            "environment": {
