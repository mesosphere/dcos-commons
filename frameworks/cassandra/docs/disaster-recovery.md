...
dcos beta-cassandra plan start backup-s3 -p SNAPSHOT_NAME= -p "CASSANDRA_KEYSPACES=" -p AWS_ACCESS_KEY_ID= -p AWS_SECRET_ACCESS_KEY= -p AWS_REGION= -p S3_BUCKET_NAME=
```
...
```
dcos beta-cassandra plan start backup-azure -p SNAPSHOT_NAME= -p "CASSANDRA_KEYSPACES=" -p CLIENT_ID= -p TENANT_ID= -p CLIENT_SECRET= -p AZURE_STORAGE_ACCOUNT= -p AZURE_STORAGE_KEY= -p CONTAINER_NAME=
```
...
AWS_REGION=us-west-2
S3_BUCKET_NAME=backups
dcos beta-cassandra plan start restore-s3 -p SNAPSHOT_NAME= -p "CASSANDRA_KEYSPACES=" -p AWS_ACCESS_KEY_ID= -p AWS_SECRET_ACCESS_KEY= -p AWS_REGION= -p S3_BUCKET_NAME=
```
...
```
dcos beta-cassandra plan start restore-azure -p SNAPSHOT_NAME= -p CLIENT_ID= -p TENANT_ID= -p CLIENT_SECRET= -p AZURE_STORAGE_ACCOUNT= -p AZURE_STORAGE_KEY= -p CONTAINER_NAME=
```
