...
public class MultiServiceManager {
  ...
  public MultiServiceManager(SchedulerConfig schedulerConfig) {
    ReadWriteLock lock =
        CycleDetectingLockUtils.newLock(schedulerConfig, MultiServiceManager.class);
  ...
  public Collection<String> getServiceNames() {
    // Alphabetical order
    Collection<String> serviceNames = new TreeSet<>();
    rlock.lock();
    try {
  ...
  public void uninstallServices(Collection<String> finishedServiceNames) {
    ...
    try {
      ...
      for (String name : finishedServiceNames) {
        ...
        AbstractScheduler uninstallScheduler =
            ((DefaultScheduler) currentService).toUninstallScheduler();
        if (isRegistered) {
