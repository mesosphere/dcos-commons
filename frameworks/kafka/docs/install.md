...
All `dcos $packagename` CLI commands have a `--name` argument allowing the user to specify which Kafka instance to query. If you do not specify a service name, the CLI assumes the default value, `kafka`. The default value for `--name` can be customized via the DC/OS CLI configuration:

```bash
$ dcos $packagename --name kafka-dev <cmd>
...
```bash
$ dcos package install --options=sample-kafka-minimal.json $packagename
```
...
```bash
$ dcos package install --options=sample-kafka-custom.json $packagename
```
...
$ dcos package install $packagename --options=kafka1.json
```
