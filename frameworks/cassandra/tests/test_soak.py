...
def test_backup_and_restore():
    ...
    with sdk_jobs.InstallJobContext(
        ...
        config.run_backup_and_restore(
            config.SERVICE_NAME, "backup-s3", "restore-s3", plan_parameters
        )
...
def test_cassandra_migration():
    ...
    backup_install_job_context = sdk_jobs.InstallJobContext(
        [
            backup_write_data_job,
            backup_verify_data_job,
            backup_delete_data_job,
    ...
    with backup_install_job_context, backup_run_job_context:
        ...
        sdk_cmd.service_request(
            "PUT", backup_service_name, "/v1/backup/start", json=backup_parameters
        )
    ...
    restore_install_job_context = sdk_jobs.InstallJobContext(
        [
            restore_write_data_job,
            restore_verify_data_job,
            restore_delete_data_job,
    ...
    with restore_install_job_context, restore_run_job_context:
        sdk_plan.start_plan(restore_service_name, "restore-s3", parameters=plan_parameters)
