...
public class CanaryStrategy implements Strategy<Step> {
  ...
  private Collection<Step> canarySteps; // SUPPRESS CHECKSTYLE VisibilityModifier
  ...
  private List<Step> interruptCanarySteps(List<Step> steps) {
    List<Step> canaryStepsInternal = steps
        .stream()
        .filter(step -> step.isPending() || step.isInterrupted())
        .limit(requiredProceeds)
        .collect(Collectors.toList());
    canaryStepsInternal.forEach(Interruptible::interrupt);
    return canaryStepsInternal;
  }

  @Override
  public Collection<Step> getCandidates(
  ...
  public static class Generator implements StrategyGenerator<Step> {
    ...
    public Strategy<Step> generate(List<Step> steps) {
      return new CanaryStrategy(
          postCanaryStrategyGenerator.generate(steps),
          requiredProceeds,
          steps
      );
    }
