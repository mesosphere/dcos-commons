 ...
 * being used.
 */
public final class StateQueries {

  static final String FILE_NAME_PREFIX = "file-";

  static final Charset FILE_ENCODING = StandardCharsets.UTF_8;

  // state store shouldn't be holding big files anyway...
  static final int FILE_SIZE_LIMIT = 1024;
    ...
    try {
      ...
      if (frameworkIdOptional.isPresent()) {
        JSONArray idArray = new JSONArray(
            Collections.singletonList(frameworkIdOptional.get().getValue())
        );
  ...
  public static Response getFile(StateStore stateStore, String name) {
    try {
      ...
      return ResponseUtils.plainResponse("Failed to get the file", Response.Status.NOT_FOUND);
    }
  }
      ...
      String key)
  {
    try {
      ...
      return ResponseUtils.jsonResponseBean(
          propertyDeserializer.toJsonString(key, stateStore.fetchProperty(key)),
          Response.Status.OK
      );
    } catch (StateStoreException ex) {
   ...
   */
  private static String getFileContent(StateStore stateStore, String fileName) {
    return new String(stateStore.fetchProperty(FILE_NAME_PREFIX + fileName), FILE_ENCODING);
  }
  ...
  static Collection<String> getFileNames(StateStore stateStore) {
    return stateStore.fetchPropertyKeys().stream()
        ...
        .map(fileName -> fileName.replaceFirst(FILE_NAME_PREFIX, ""))
   ...
   */
  private static String getZoneFromTaskNameAndIP(
      StateStore stateStore,
      String podType,
      String ipAddress)
  {
