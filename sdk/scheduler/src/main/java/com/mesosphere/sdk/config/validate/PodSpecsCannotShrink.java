...
public class PodSpecsCannotShrink implements ConfigValidator<ServiceSpec> {
      ...
      ServiceSpec newConfig)
  {
    ...
    for (PodSpec oldPod : oldConfig.get().getPods()) {
      ...
      if (newPod == null) {
        // Prior pod of this name is now missing. Allow the removal only if the old pod's
        // allow-decommission bit was set. This could be used as an (admittedly convoluted)
        // path for migrating away from an old service layout, with an interim release
        // marking a pod with allow-decommission before it's removed entirely.
        if (!oldPod.getAllowDecommission()) {
          errors.add(ConfigValidationError.transitionError(
              ...
              "null",
              String.format("New config is missing PodSpec named '%s'" +
                      " (expected present with >= %d tasks)",
      ...
      if (newPod.getCount() < oldPod.getCount() && !newPod.getAllowDecommission()) {
        ...
        // Only allow the new pod to be shrunk if its allow-decommission bit is set. We
        // intentionally check allow-decommission against the new pod instead of the old
        // pod with the following reasoning:
        // - Older version disallows, newer version allows: Allow count to decrease now
        // that it's supported
        // - Older version allows, newer version disallows: Disallow count from
        // decreasing now that it's no longer supported
        errors.add(ConfigValidationError.transitionError(
            String.format("PodSpec[name:%s]", newPod.getType()),
            String.valueOf(oldPod.getCount()),
            String.valueOf(newPod.getCount()),
            String.format(
                "New config's PodSpec named '%s' has %d tasks, expected >=%d tasks",
