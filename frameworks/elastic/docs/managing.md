...
```bash
$ dcos elastic --name="elastic" describe > options.json
```
...
```bash
$ dcos elastic --name="elastic" update start --options=options.json
```
...
To see a full listing of available options, run `dcos package describe --config elastic` in the CLI, or browse the Beta Elastic install dialog in the DC/OS web interface.
...
For example:
```bash
$ dcos elastic --name=<service-name> pod info master-0
```
