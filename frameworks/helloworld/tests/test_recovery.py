...
@pytest.mark.skip(reason="INFINITY-2024")
...
def test_pods_restart():
    ...
    stdout = sdk_cmd.run_cli('hello-world pods info hello-0')
    old_agent = json.loads(stdout)[0]['info']['slaveId']['value']

    stdout = sdk_cmd.run_cli('hello-world pods restart hello-0')
...
def test_pods_replace():
    ...
    # get current agent id:
    stdout = sdk_cmd.run_cli('hello-world pods info world-0')
    old_agent = json.loads(stdout)[0]['info']['slaveId']['value']

    jsonobj = json.loads(sdk_cmd.run_cli('hello-world pods replace world-0'))
    assert len(jsonobj) == 2
