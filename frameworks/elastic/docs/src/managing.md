...
```bash
$ dcos $packagename --name="elastic" describe > options.json
```
...
```bash
$ dcos $packagename --name="elastic" update start --options=options.json
```
...
To see a full listing of available options, run `dcos package describe --config $packagename` in the CLI, or browse the Beta Elastic install dialog in the DC/OS web interface.
...
For example:
```bash
$ dcos $packagename --name=<service-name> pod info master-0
```
...
```bash
$ dcos $packagename pod info data-0
```
