...
```bash
$ curl -H "Authorization:token=" <dcos_url>/service/hdfs/v1/plans/deploy
```
## Pause Installation
...
```bash
$ curl -X POST -H "Authorization:token=" <dcos_url>/service/hdfs/v1/plans/deploy/interrupt
```
...
```bash
$ curl -H "Authorization:token=" dcos_url/service/hdfs/v1/endpoints/hdfs-site.xml
```

```bash
$ curl -H "Authorization:token=" <dcos_url>/service/hdfs/v1/endpoints/core-site.xml
```
...
HTTP Example
```bash
$ curl  -H "Authorization:token=" <dcos_url>/service/hdfs/v1/pod
[
    "data-0",
    "data-1",
    "data-2",
...
```bash
$ curl  -H "Authorization:token=" <dcos_url>/service/hdfs/v1/pod/<node-id>/info
```
...
HTTP Example
```bash
$ curl  -H "Authorization:token=" <dcos_url>/service/hdfs/v1/pod/journal-0/info
[{
	info: {
		name: "journal-0-node",
		taskId: {
		...
		executor: {
			...
			command: {
				...
				environment: null,
				shell: null,
				value: "export LD_LIBRARY_PATH=/libmesos-bundle/lib: && export MESOS_NATIVE_JAVA_LIBRARY=$(ls /libmesos-bundle/lib/libmesos-*.so) && export JAVA_HOME=$(ls -d /jre*/) && ./executor/bin/executor",
				arguments: [],
				user: null
			},
			container: null,
...
HTTP Example
```bash
$ curl -X POST -H "Authorization:token=" <dcos_url>/service/hdfs/v1/pod/<node-id>/replace
```
...
HTTP Example
```bash
$ curl -X POST -H "Authorization:token=" <dcos_url>/service/hdfs/v1/pod/<node-id>/restart
```
...
HTTP Example
```bash
$ curl -H "Authorization:token=" <dcos_url>/service/hdfs/v1/configurations/target
{
	name: "hdfs",
	role: "hdfs-role",
	principal: "hdfs-principal",
	...
	pod - specs: [{
		...
		task - specs: [{
			...
			readiness - check - spec: {
				command: "./hadoop-2.6.0-cdh5.9.1/bin/hdfs haadmin -getServiceState name--node",
				delay: 0,
				interval: 5,
				timeout: 60
			},
...
HTTP Example
```bash
$ curl -H "Authorization:token=" <dcos_url>/service/hdfs/v1/configurations
[
...
HTTP Example
```bash
$ curl -H "Authorization:token=" <dcos_url>/service/hdfs/v1/configurations/9a8d4308-ab9d-4121-b460-696ec3368ad6
{
...
```bash
$ curl -H "Authorization: token=" "<dcos_url>/service/hdfs/v1/state/properties/suppressed"
```
