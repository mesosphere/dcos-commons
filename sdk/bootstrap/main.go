...
func verifySelfResolution(podIP string, resolveTimeout time.Duration) {
	...
	if err != nil {
		printEnv()
		log.Printf("Missing required envvars to build task DNS address. " +
			"Ensure that TASK_NAME and FRAMEWORK_HOST are both set or " +
			"disable self resolution with --self-resolve=false")
	}
	log.Printf("Verifying that %s resolves to %s", taskHost, podIP)
...
func main() {
	...
	if args.getTaskIP {
		log.Printf("Printing new task IP: %s", podIP)
		fmt.Printf("%s", podIP)
		os.Exit(0)
	}
