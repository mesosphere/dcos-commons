...
public enum GoalStateOverride {
  ...
  GoalStateOverride(String serializedName, String pendingName) {
    // Validation: By convention all state labels must be all caps/no spaces
    Pattern validLabelPattern = Pattern.compile("^[A-Z_]+$");
    ...
    for (GoalState goalState : GoalState.values()) {
      if (goalState.toString().equals(serializedName)) {
        throw new IllegalArgumentException(
            String.format(
                "Provided GoalStateOverride serialized name '%s' collides with an existing " +
                    "GoalState=%s",
                serializedName,
                Arrays.asList(GoalState.values())
            )
        );
      }
  ...
  public enum Progress {
    ...
    Progress(String serializedName) {
      this.serializedName = serializedName;
    }
