...
@pytest.mark.soak
def test_soak_load(service_name, scenario, package_version) -> None:
    ...
    _launch_load(soak_service_name, scenario, package_version)


@pytest.mark.scale
def test_scaling_load(
    service_name, scenario, service_count, min_index, max_index, batch_size, package_version
) -> None:
    ...
    if min_index == -1 or max_index == -1:
        deployment_list = [
            "{}-{}".format(scale_service_name, index) for index in range(0, int(service_count))
        ]
        min_index = 0
        max_index = service_count
    else:
        deployment_list = [
            "{}-{}".format(scale_service_name, index) for index in range(min_index, max_index)
        ]
...
def _launch_load(service_name, scenario, package_version) -> None:
    ...
    _install_service(launch_service_name, scenario, security_info, package_version)
