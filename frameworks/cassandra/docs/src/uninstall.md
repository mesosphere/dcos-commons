...
```bash
dcos package uninstall --app-id=cassandra-dev $packagename
```
...
1. Stop the service. From the DC/OS CLI, enter `dcos package uninstall --app-id=<instancename> <packagename>`.
   For example, `dcos package uninstall --app-id=cassandra-dev $packagename`.
...
$ dcos package uninstall --app-id=$MY_SERVICE_NAME $packagename`.
