...
def test_canary_init():
    def fn():
        return cmd.run_cli('helloworld pods list')
...
def test_canary_first():
    cmd.run_cli('helloworld plan continue deploy hello-deploy')

    expected_tasks = ['hello-0']
    tasks.check_running(PACKAGE_NAME, len(expected_tasks))
    assert json.loads(cmd.run_cli('helloworld pods list')) == expected_tasks
...
def test_canary_plan_continue_noop():
    cmd.run_cli('helloworld plan continue deploy')
...
def test_canary_second():
    cmd.run_cli('helloworld plan continue deploy world-deploy')
