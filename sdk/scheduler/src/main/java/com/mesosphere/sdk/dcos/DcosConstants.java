 ...
 */
public final class DcosConstants {

  public static final String MESOS_MASTER = "master.mesos";

  public static final String MESOS_MASTER_URI = httpPrefix(MESOS_MASTER);

  public static final String DEFAULT_SECRET_STORE_URI =
      MESOS_MASTER_URI + "/secrets/v1/secret/default/";
  ...
  // These ports should be available to a container on the overlay regardless of it's permissions,
  // it's unlikely that a pod will ever exceed 1000 ports.
  ...
  public static boolean networkSupportsPortMapping(String networkName) {
    switch (networkName) {
      ...
      case DEFAULT_OVERLAY_NETWORK:
        return false;
      default:
        // Here we decide whether to automatically map ports (ContainerIP:port : HostIP:port) when
        // joining a network that does not have an explicit port-mapping capability. It seems that
        // port mapping (bridge networking) is in fact not common so we default to disabled.
        return false;
    }
  }
  ...
  public static void warnIfUnsupportedNetwork(String networkName) {
    if (!SUPPORTED_OVERLAY_NETWORKS.contains(networkName)) {
      LOGGER.warn(
          "Virtual network '{}' is not supported, unexpected behavior may result" +
              " (expected one of: {})",
          networkName,
          SUPPORTED_OVERLAY_NETWORKS
      );
    }
  }

  private static String httpPrefix(String host) {
    return "http://" + host;
  }
}
