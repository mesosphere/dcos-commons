...
def test_universe_file_formatting(file_base):
    ...
    with open(path, "r") as source:
        ...
        formatted_data = [l for l in json.dumps(json.loads("".join(raw_data), object_pairs_hook=collections.OrderedDict), indent=2).split(
            "\n")]
        diff = list(difflib.unified_diff(raw_data, formatted_data, fromfile=path, tofile="formatted"))
        if diff:
            print("\n" + ("\n".join(diff)))
