...
def test_canary_init():
    ...
    pl = sdk_plan.wait_for_plan_status(
        config.SERVICE_NAME, 'deploy', 'WAITING')
    log.info(pl)
...
def test_canary_third():
    ...
    pl = sdk_plan.wait_for_completed_phase(
        config.SERVICE_NAME, 'deploy', 'hello-deploy')
    log.info(pl)
...
def test_increase_count():
    ...
    pl = sdk_plan.wait_for_plan_status(
        config.SERVICE_NAME, 'deploy', 'COMPLETE')
    log.info(pl)
...
def test_increase_cpu():
    ...
    assert sdk_cmd.svc_cli(config.PACKAGE_NAME, config.SERVICE_NAME,
                           'pod list', json=True) == expected_tasks
    assert hello_0_ids == sdk_tasks.get_task_ids(
    ...
    sdk_tasks.check_tasks_updated(
        config.SERVICE_NAME, 'hello-0-server', hello_0_ids)
    sdk_tasks.check_running(config.SERVICE_NAME, len(expected_tasks))
    ...
    sdk_cmd.svc_cli(config.PACKAGE_NAME, config.SERVICE_NAME,
                    'plan continue deploy hello-deploy')
    sdk_tasks.check_tasks_updated(
        config.SERVICE_NAME, 'hello-1-server', hello_1_ids)
