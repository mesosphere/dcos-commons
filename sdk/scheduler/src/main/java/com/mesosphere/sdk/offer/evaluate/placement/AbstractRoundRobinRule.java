...
abstract class AbstractRoundRobinRule implements PlacementRule {
  ...
  protected AbstractRoundRobinRule(StringMatcher taskFilter, Optional<Integer> distinctKeyCount) {
    ...
    if (taskFilter == null) {
      taskFilter = AnyMatcher.create(); // SUPPRESS CHECKSTYLE ParameterAssignment
    }
      ...
      Collection<TaskInfo> tasks)
  {
    ...
    if (offerKey == null) {
      ...
      return EvaluationOutcome.fail(
          this,
          "Offer lacks needed information for round robin placement"
      ).build();
    }
        ...
        || offerKeyCount <= minKnownKeyCount)
    {
      ...
      if (!distinctKeyCount.isPresent()) {
        ...
        return EvaluationOutcome.pass(
            this,
            "Distinct key count is unspecified, and '%s' has %d instances while others" +
                " have%d to %d",
            offerKey,
            offerKeyCount,
            minKnownKeyCount,
