...
- Packaging
  ...
  {{ include.package_name }} is packaged for deployment on DC/OS. DC/OS packages follow the [Universe schema](https://github.com/mesosphere/universe), which defines how packages expose customization options at initial installation. When a package is installed on the cluster, the packaging service (named 'Cosmos') creates a Marathon app that contains a rendered version of the `marathon.json.mustache` template provided by the package. For {{ include.package_name }}, this Marathon app is the scheduler for the service.
...
Internally, {{ include.package_name }} treats "Deployment" as moving from one state to another state. By this definition, "Deployment" applies to many scenarios:

- When {{ include.package_name }} is first installed, deployment is moving from a null configuration to a deployed configuration.
...
1. The user runs `dcos package install {{ include.package_name }}` in the DC/OS CLI or clicks `Install` for a given package on the DC/OS Dashboard.
...
1. Cosmos creates a Marathon app definition by rendering {{ include.package_name }}'s `marathon.json.mustache` with the configuration options provided in the request, which represents {{ include.package_name }}'s Scheduler. Cosmos queries Marathon to create the app.

1. Marathon launches the {{ include.package_name }}'s scheduler somewhere in the cluster using the rendered app definition provided by Cosmos.

1. {{ include.package_name }}'s scheduler is launched. From this point onwards, the SDK handles deployment.

#### Steps handled by the Scheduler

{{ include.package_name }}'s `main()` function is run like any other Java application. The scheduler starts with the following state:
...
1. Now that the Scheduler has registered as a Mesos Framework, it is able to start interacting with Mesos and receiving offers. When this begins, the scheduler will begin running the [Offer Cycle](#offer-cycle) and deploying {{ include.package_name }}. See that section for more information.
...
This is the flow for uninstalling {{ include.package_name }}.
...
{{ include.package_name }} performs the following operations as Offers are received from Mesos:
