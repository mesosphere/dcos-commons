...
def fault_domain_vars_are_present(pod_instance):
    info = sdk_cmd.service_request(
        'GET', config.SERVICE_NAME, '/v1/pod/{}/info'.format(pod_instance)).json()[0]['info']
    variables = info['command']['environment']['variables']
    region = next(
        (var for var in variables if var['name'] == 'REGION'), ['NO_REGION'])
    zone = next(
        (var for var in variables if var['name'] == 'ZONE'), ['NO_ZONE'])
...
def succeed_placement(options):
    ...
    sdk_install.install(config.PACKAGE_NAME,
                        config.SERVICE_NAME, 0, additional_options=options)
    sdk_install.uninstall(config.PACKAGE_NAME, config.SERVICE_NAME)
...
def test_cluster():
    ...
    ensure_count_per_agent(
        hello_count=config.get_num_private_agents(), world_count=0)
...
def setup_constraint_switch():
    ...
    sdk_install.install(config.PACKAGE_NAME,
                        config.SERVICE_NAME, 1, additional_options=options)
...
def get_task_host(task_name):
    ...
    raise Exception("Unable to match the task reported in CLI: {} to tasks from get_summary: {}".format(
