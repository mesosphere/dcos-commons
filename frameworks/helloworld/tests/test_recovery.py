...
def test_kill_hello_node():
    ...
    tasks.kill_task_with_pattern('hello', 'hello-0-server.helloworld.mesos')
...
def test_pods_restart():
    ...
    stdout = cmd.run_cli('helloworld pods info hello-0')
    old_agent = json.loads(stdout)[0]['info']['slaveId']['value']

    stdout = cmd.run_cli('helloworld pods restart hello-0')
...
def test_pods_replace():
    ...
    # get current agent id:
    stdout = cmd.run_cli('helloworld pods info world-0')
    old_agent = json.loads(stdout)[0]['info']['slaveId']['value']

    jsonobj = json.loads(cmd.run_cli('helloworld pods replace world-0'))
    assert len(jsonobj) == 2
