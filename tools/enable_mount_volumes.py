...
def create_volume(client, zone):
    response = client.create_volume(
        Size=24, AvailabilityZone=zone, VolumeType="gp2", Encrypted=False
    )
...
def attach_volume(client, volume_id, instance_id, device="/dev/xvdm"):
    response = client.attach_volume(VolumeId=volume_id, InstanceId=instance_id, Device=device)
...
def configure_delete_on_termination(client, volume_id, instance_id, device="/dev/xvdm"):
    response = client.modify_instance_attribute(
        ...
        BlockDeviceMappings=[
            {"DeviceName": device, "Ebs": {"VolumeId": volume_id, "DeleteOnTermination": True}}
        ],
    )
...
def tag_volume(client, volume_id):
    response = client.create_tags(
        Resources=[volume_id],
        Tags=[{"Key": "ccm_volume_name", "Value": "infinity-" + str(uuid.uuid1())}],
    )
...
def detach_volume(client, volume_id, instance_id, device="/dev/xvdm"):
    response = client.detach_volume(VolumeId=volume_id, InstanceId=instance_id, Device=device)
...
def configure_partition(device, partition_index, start, end, stdout):
    ...
    run("sudo parted -s {} mkpart primary ext4 {} {}".format(device, start, end), stdout=stdout)
    run("sudo mkfs -t ext4 {}".format(device_partition), stdout=stdout)
    run("sudo mkdir -p {}".format(mount_location), stdout=stdout)
    run("sudo mount {} {}".format(device_partition, mount_location), stdout=stdout)
    run(
        "sudo sh -c \"echo '{} {} ext4 defaults 0 2' >> /etc/fstab\"".format(
            device_partition, mount_location
        ),
        stdout=stdout,
    )
...
def configure_device(device="/dev/xvdm", stdout=sys.stdout):
    ...
    run(
        'until [[ "$(lsblk -o NAME -r | grep {} | wc -l)" -gt "0" ]]; do echo "Waiting for {}"; sleep 2; done'.format(
...
def main(stack_id="", stdout=sys.stdout):
    ...
    if (
        aws_profile_key not in os.environ
        or aws_region_key not in os.environ
        or stack_id_key not in os.environ
    ):
        logger.error(
            "{}, {} and {} envvars are required.".format(
    ...
    if not reservations:
        ...
        return 1
    logger.info(
        "Found {} reservations with {} = {}".format(len(reservations), stack_id_key, stack_id)
    )
    ...
    for instance in private_instances:
        ...
        while attempts < max_attempts:
            ...
            try:
                ...
                logger.info(
                    "Volume: {} is now attached to instance: {}".format(volume_id, instance_id)
                )
                break
            except botocore.exceptions.WaiterError as e:
                logger.error("Error occured: {}".format(e))
