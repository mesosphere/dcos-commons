...
def configure_package(configure_security):
    try:
        ...
        options = {
            "service": {
                "yaml": "nonessential_tasks"
            }
        }

        sdk_install.install(config.PACKAGE_NAME,
                            config.SERVICE_NAME, 2, additional_options=options)
...
def test_kill_essential():
    ...
    # leave files as-is for the next test
    verify_shared_executor('hello-0', delete_files=False)
...
def test_kill_nonessential():
    ...
    old_essential_ids = sdk_tasks.get_task_ids(
        config.SERVICE_NAME, 'hello-0-essential')
    assert len(old_essential_ids) == 1
    old_nonessential_ids = sdk_tasks.get_task_ids(
        config.SERVICE_NAME, 'hello-0-nonessential')
    ...
    sdk_tasks.check_tasks_updated(
        config.SERVICE_NAME, 'hello-0-nonessential', old_nonessential_ids)
    sdk_plan.wait_for_completed_recovery(config.SERVICE_NAME)
    sdk_tasks.check_tasks_not_updated(
        config.SERVICE_NAME, 'hello-0-essential', old_essential_ids)
...
def verify_shared_executor(pod_name, expected_files=['essential', 'nonessential'], delete_files=True):
    ...
    for task_name in task_names:
        ...
        filenames = sdk_cmd.run_cli(
            'task ls {} shared-volume/'.format(task_name)).strip().split()
    ...
    if delete_files:
        ...
        sdk_cmd.task_exec(
            task_names[0], 'rm ' + ' '.join([os.path.join(volume_dir, name) for name in filenames]))
