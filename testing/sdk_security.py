...
def create_service_account(service_account_name: str, service_account_secret: str) -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        private_key_name = os.path.join(d, "private-key.pem")
        public_key_name = os.path.join(d, "public-key.pem")

        log.info('Create keypair')
        create_keypair_command = [
            "security", "org", "service-accounts",
            "keypair", private_key_name, public_key_name,
        ]
        sdk_cmd.run_cli(" ".join(create_keypair_command))

        log.info('Create service account')
        create_service_account_command = [
            "security", "org", "service-accounts", "create",
            "-p", public_key_name,
            "-d", '"Service account for integration tests"',
            '"{}"'.format(service_account_name),
        ]
        sdk_cmd.run_cli(" ".join(create_service_account_command))

        log.info('Create secret')
        create_secret_command = [
            "security", "secrets", "create-sa-secret",
            "--strict", private_key_name,
            '"{}"'.format(service_account_name),
            '"{}"'.format(service_account_secret),
        ]
        sdk_cmd.run_cli(" ".join(create_secret_command))
