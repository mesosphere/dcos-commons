...
def default_populated_index():
    ...
    config.create_index(config.DEFAULT_INDEX_NAME,
                        config.DEFAULT_SETTINGS_MAPPINGS, service_name=foldered_name)
...
def test_pod_replace_then_immediate_config_update():
    ...
    config.check_elasticsearch_plugin_installed(
        plugin_name, service_name=foldered_name)
...
def test_custom_yaml_base64():
    ...
    config.update_app(foldered_name, {
                      'CUSTOM_YAML_BLOCK_BASE64': base64_str}, current_expected_task_count)
...
def test_xpack_toggle_with_kibana(default_populated_index):
    ...
    elasticsearch_url = "http://" + \
        sdk_hosts.vip_host(foldered_name, "coordinator", 9200)
    sdk_install.install(
        config.KIBANA_PACKAGE_NAME,
        config.KIBANA_PACKAGE_NAME,
        0,
    ...
    config.update_app(
        foldered_name, {'UPDATE_STRATEGY': 'serial'}, current_expected_task_count)
...
def test_losing_and_regaining_index_health(default_populated_index):
    ...
    shakedown.kill_process_on_host(sdk_hosts.system_host(
        foldered_name, "data-0-node"), "data__.*Elasticsearch")
    config.check_elasticsearch_index_health(
...
def test_master_reelection():
    initial_master = config.get_elasticsearch_master(
        service_name=foldered_name)
    shakedown.kill_process_on_host(sdk_hosts.system_host(
        foldered_name, initial_master), "master__.*Elasticsearch")
...
def test_plugin_install_and_uninstall(default_populated_index):
    plugin_name = 'analysis-phonetic'
    config.update_app(foldered_name, {
                      'TASKCFG_ALL_ELASTICSEARCH_PLUGINS': plugin_name}, current_expected_task_count)
    config.check_elasticsearch_plugin_installed(
        plugin_name, service_name=foldered_name)

    config.update_app(foldered_name, {
                      'TASKCFG_ALL_ELASTICSEARCH_PLUGINS': ''}, current_expected_task_count)
...
def test_unchanged_scheduler_restarts_without_restarting_tasks():
    ...
    shakedown.kill_process_on_host(sdk_marathon.get_scheduler_host(
        foldered_name), "elastic.scheduler.Main")
...
def test_adding_data_node_only_restarts_masters():
    ...
    sdk_tasks.check_tasks_updated(
        foldered_name, "master", initial_master_task_ids)
    sdk_tasks.check_tasks_not_updated(
        foldered_name, "data", initial_data_task_ids)
    sdk_tasks.check_tasks_not_updated(
        foldered_name, "coordinator", initial_coordinator_task_ids)
    sdk_plan.wait_for_completed_deployment(foldered_name)
    sdk_plan.wait_for_completed_recovery(foldered_name)
