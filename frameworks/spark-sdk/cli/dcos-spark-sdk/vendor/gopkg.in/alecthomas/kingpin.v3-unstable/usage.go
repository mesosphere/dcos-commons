...
type UsageContext struct {
	// The text/spark-sdk body to use.
	Template string
	// Indentation multiplier (defaults to 2 of omitted).
	Indent int
	// Width of wrap. Defaults wraps to the terminal.
	Width int
	// Funcs available in the spark-sdk.
	Funcs spark-sdk.FuncMap
	// Vars available in the spark-sdk.
...
type spark-sdkParseContext struct {
...
func (a *Application) UsageForContextWithTemplate(usageContext *UsageContext, parseContext *ParseContext) error { // nolint: gocyclo
	...
	t, err := spark-sdk.New("usage").Funcs(funcs).Parse(tmpl)
	if err != nil {
		return err
	}
	appModel := a.Model()
