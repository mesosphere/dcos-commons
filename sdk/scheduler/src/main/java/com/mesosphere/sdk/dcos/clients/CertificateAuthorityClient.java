...
public class CertificateAuthorityClient {
  ...
  public X509Certificate sign(byte[] csr) throws Exception {
    ...
    if (!data.getBoolean("success")) { // SUPPRESS CHECKSTYLE MultipleStringLiterals
  ...
  private JSONObject doPostRequest(String path, JSONObject data) throws Exception {
    HttpResponse httpResponse = httpExecutor
        .execute(
            Request
                .Post(new URI(DcosConstants.CA_BASE_URI + path))
                .bodyString(data.toString(), ContentType.APPLICATION_JSON)
        )
        .returnResponse();
    handleResponseStatusLine(httpResponse.getStatusLine(), HttpStatus.SC_OK);
