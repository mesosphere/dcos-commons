...
public class PodsResourceTest {
    ...
    static {
        ...
        TaskUtils.setType(infoBuilder, "test");
        TaskUtils.setIndex(infoBuilder, 0);
        POD_0_TASK_A = infoBuilder.setName("a").setTaskId(TaskUtils.toTaskId("a")).build();
        POD_0_STATUS_A = TaskTestUtils.generateStatus(POD_0_TASK_A.getTaskId(), TaskState.TASK_RUNNING);

        POD_0_TASK_B = POD_0_TASK_A.toBuilder().setName("b").setTaskId(TaskUtils.toTaskId("b")).build();
        POD_0_STATUS_B = TaskTestUtils.generateStatus(POD_0_TASK_B.getTaskId(), TaskState.TASK_STAGING);

        POD_0_TASK_C = POD_0_TASK_A.toBuilder().setName("c").setTaskId(TaskUtils.toTaskId("c")).build();
        POD_0_STATUS_C = TaskTestUtils.generateStatus(POD_0_TASK_C.getTaskId(), TaskState.TASK_RUNNING);

        POD_0_TASK_D = POD_0_TASK_A.toBuilder().setName("d").setTaskId(TaskUtils.toTaskId("d")).build();

        // pod 1
        infoBuilder = POD_0_TASK_A.toBuilder();
        TaskUtils.setIndex(infoBuilder, 1);
        POD_1_TASK_A = infoBuilder.setName("a").setTaskId(TaskUtils.toTaskId("a")).build();
        POD_1_STATUS_A = TaskTestUtils.generateStatus(POD_1_TASK_A.getTaskId(), TaskState.TASK_FINISHED);

        POD_1_TASK_B = POD_1_TASK_A.toBuilder().setName("b").setTaskId(TaskUtils.toTaskId("b")).build();
        POD_1_STATUS_B = TaskTestUtils.generateStatus(POD_1_TASK_B.getTaskId(), TaskState.TASK_RUNNING);

        // pod 2
        infoBuilder = POD_0_TASK_A.toBuilder();
        TaskUtils.setIndex(infoBuilder, 2);
        POD_2_TASK_A = infoBuilder.setName("a").setTaskId(TaskUtils.toTaskId("a")).build();
