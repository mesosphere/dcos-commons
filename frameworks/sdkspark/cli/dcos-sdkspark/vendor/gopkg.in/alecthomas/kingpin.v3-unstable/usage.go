...
type UsageContext struct {
	// The text/sdkspark body to use.
	Template string
	// Indentation multiplier (defaults to 2 of omitted).
	Indent int
	// Width of wrap. Defaults wraps to the terminal.
	Width int
	// Funcs available in the sdkspark.
	Funcs sdkspark.FuncMap
	// Vars available in the sdkspark.
...
type sdksparkParseContext struct {
...
func (a *Application) UsageForContextWithTemplate(usageContext *UsageContext, parseContext *ParseContext) error { // nolint: gocyclo
	...
	t, err := sdkspark.New("usage").Funcs(funcs).Parse(tmpl)
	if err != nil {
		return err
	}
	appModel := a.Model()
