 ...
 */
public final class EndpointsQueries {
      ...
      SchedulerConfig schedulerConfig)
  {
    try {
      ...
      return ResponseUtils.jsonOkResponse(new JSONArray(endpoints));
      ...
      SchedulerConfig schedulerConfig)
  {
    try {
      ...
      if (customValue != null) {
        ...
        return ResponseUtils.plainOkResponse(customValue.getEndpoint());
      }

      // Fall back to checking default values:
      JSONObject endpoint = getDiscoveryEndpoints(stateStore, frameworkName, schedulerConfig)
          .get(endpointName);
      if (endpoint != null) {
        return ResponseUtils.jsonOkResponse(endpoint);
      }
      ...
      String ipHostPort)
  {
    ...
    Collection<EndpointUtils.VipInfo> vips =
        AuxLabelAccess.getVIPsFromLabels(taskName, taskInfoPort);
  ...
  private static void addPortAndVipToEndpoints(
      ...
      String vipHostPort)
  {
    JSONObject vipEndpoint = addPortToEndpoints(
        endpointsByName,
        portName,
        autoipHostPort,
        ipHostPort);
