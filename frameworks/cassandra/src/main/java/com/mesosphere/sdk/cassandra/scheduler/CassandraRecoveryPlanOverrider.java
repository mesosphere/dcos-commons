...
public class CassandraRecoveryPlanOverrider implements RecoveryPlanOverrider {
    ...
    public CassandraRecoveryPlanOverrider(TaskStore taskStore, Plan replaceNodePlan) {
    ...
    private Phase getNodeRecoveryPhase(Plan inputPlan, int index) {
        ...
        Optional<String> replaceIp = taskStore.getTaskIp(TaskSpec.getInstanceName(podInstance, taskSpec));
        if (!replaceIp.isPresent()) {
        ...
        builder.value(String.format(
                ...
                command.getValue().trim(), replaceIp.get()));
        ...
        Step replaceStep = inputLaunchStep.withPodInstanceRequirement(replacePodInstanceRequirement);
        ...
        if (CassandraSeedUtils.isSeedNode(replaceIndex)) {
            ...
            List<Step> restartSteps = inputPhase.getChildren().stream()
                    ...
                    // Set recoveryType to TRANSIENT:
                    .map(step -> step.withPodInstanceRequirement(
                            PodInstanceRequirement.newBuilder(step.getPodInstanceRequirement().get())
