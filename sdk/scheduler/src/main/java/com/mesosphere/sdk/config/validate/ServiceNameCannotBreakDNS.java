...
/**
 * This validator enforces that the specified service name (e.g. /foo/bar/service) does not exceed
 * 63 characters with slashes removed. This is to ensure the DNS subdomain of the service does
 * not exceed the character limit specified in https://www.ietf.org/rfc/rfc1035.txt.
 * <p>
...
public class ServiceNameCannotBreakDNS implements ConfigValidator<ServiceSpec> {
      ...
      ServiceSpec newConfig)
  {
    ...
    if (oldConfig.isPresent()) {
      ...
      if (nameExceedsSubDomainLimit(newConfig.getName())) {
        LOGGER.warn("The service name (without slashes) exceeds the maximum size allowed in a DNS" +
            " subdomain.");
      }
    } else if (nameExceedsSubDomainLimit(newConfig.getName())) {
      ...
      return Collections.singletonList(ConfigValidationError.valueError(
          "service.name",
          newConfig.getName(),
          "Service name (without slashes) exceeds 63 characters. In order for service DNS to " +
              "work correctly, the service name (without slashes) must not exceed 63 characters"));
    }
    return Collections.emptyList();
  }

  private boolean nameExceedsSubDomainLimit(String name) {
