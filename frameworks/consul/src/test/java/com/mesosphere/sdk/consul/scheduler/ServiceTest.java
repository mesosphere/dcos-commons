package com.mesosphere.sdk.consul.scheduler;
...
public class ServiceTest {
    ...
    private static Collection<SimulationTick> getDeploymentTicks() {
        ...
        ticks.add(Send.offerBuilder("consul").build());
        ticks.add(Expect.launchedTasks("consul-0-node"));
        ticks.add(Send.taskStatus("consul-0-node", Protos.TaskState.TASK_ERROR).build());
        ...
        ticks.add(Send.offerBuilder("consul").setPodIndexToReoffer(0).build());
        ticks.add(Expect.launchedTasks("consul-0-node"));
        ticks.add(Send.taskStatus("consul-0-node", Protos.TaskState.TASK_RUNNING).build());
