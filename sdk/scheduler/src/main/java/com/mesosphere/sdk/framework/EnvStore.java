...
public class EnvStore {
  ...
  /**
   * If the value cannot be parsed as an int, this points to the source envKey, and ensures
   * that calls only throw {@link ConfigException}.
   */
  private static int toInt(String envKey, String envVal) {
    try {
      ...
      throw ConfigException.invalidValue(String.format(
          "Failed to parse configured environment variable '%s' as an integer: %s",
          envKey,
          envVal));
    }
  }
   ...
   * calls only throw {@link ConfigException}.
   */
  private static long toLong(String envKey, String envVal) {
    try {
      ...
      throw ConfigException.invalidValue(String.format(
          "Failed to parse configured environment variable '%s' as a long integer: %s",
          envKey,
          envVal));
    }
  }
  ...
  /**
   * If the value cannot be parsed as a double, this points to the source envKey, and ensures
   * that calls only throw {@link ConfigException}.
   */
  ...
  private static boolean toBoolean(String envKey, String envVal) {
    ...
    switch (envVal.trim().charAt(0)) {
      ...
      default:
        throw ConfigException.invalidValue(String.format(
            "Failed to parse configured environment variable '%s' as a boolean: %s",
            envKey,
            envVal));
    }
  }
  ...
  /**
   * Returns the requested value if set, or {@code defaultValue} if it's missing from the map.
   */
  ...
  /**
   * Returns the requested value if set and non-empty, or {@code defaultValue} if it's missing from
   * the map or is empty or whitespace.
   */
  ...
  /**
   * Returns the requested value if set, or throws an exception if it's missing from the map.
   */
  public String getRequired(String envKey) {
    String value = envMap.get(envKey);
    if (value == null) {
      throw ConfigException.notFound(String.format(
