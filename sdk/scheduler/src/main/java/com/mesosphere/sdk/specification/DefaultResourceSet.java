...
public final class DefaultResourceSet implements ResourceSet {
  ...
  public static final class Builder {
    ...
    public final String preReservedRole;
    ...
    private Builder addScalarResource(Double r, String resourceId) {
      ...
      if (resources
          .stream()
          .anyMatch(resourceSpecification ->
              Objects.equals(resourceSpecification.getName(), resourceId)))
      {
        ...
        List<String> profiles)
    {
              ...
              Objects.equals(volumeSpecification.getContainerPath(), containerPath)))
      {
        throw new IllegalStateException(
            "Cannot configure multiple volumes with the same containerPath"
        );
      }
      ...
      if (volumeTypeEnum == VolumeSpec.Type.ROOT) {
        if (!profiles.isEmpty()) {
          throw new IllegalArgumentException(String.format(
              "Provided volume type '%s' for path '%s' cannot have profiles",
              volumeType,
              containerPath)
          );
        }
