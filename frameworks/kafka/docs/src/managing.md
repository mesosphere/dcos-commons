...
To see a full listing of available options, run `dcos package describe --config $packagename` in the CLI, or browse the Beta Kafka install dialog in the DC/OS web interface.
...
```bash
$ dcos $packagename --name=/kafka update --options=kafka-options.json
```
