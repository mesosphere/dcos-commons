...
class UniversePackageBuilder(object):
    ...
    def content_type(self):
        ...
        package_filepath = os.path.join(self._input_dir_path, 'package.json')
        package_json = json.loads(open(package_filepath).read(), object_pairs_hook=collections.OrderedDict)
        packaging_version_major = package_json['packagingVersion'].split('.')[0]

        return 'application/vnd.dcos.universe.repo+json;charset=utf-8;version=v{}'.format(packaging_version_major)
