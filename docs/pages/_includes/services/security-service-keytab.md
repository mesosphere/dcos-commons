...
The DC/OS {{ include.techName }} service uses a keytab containing all node principals (service keytab) to simplify orchestration. After creating the principals above, generate the service keytab making sure to include all the broker principals. This will be stored as a secret in the DC/OS Secret Store.
...
The service keytab should be stored at `service/path/name/service.keytab` (as noted above for 1.10, it would be `__dcos_base64__service.keytab`), where `service/path/name` matches the path and name of the service. For example, if installing with the options
```json
{
    "service": {
        "name": "a/good/example"
