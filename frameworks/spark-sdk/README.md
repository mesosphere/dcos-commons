This spark-sdk README may be used as a starting point for writing a Service Guide for your DC/OS Service.
...
First, we'll fetch the default application's environment, current application's environment, and the actual spark-sdk that maps config values to the environment:
...
```
1. We'll also get the entire application spark-sdk:
```bash
...
```
1. Now compare these values to the values contained in the `env` section in application spark-sdk:
```bash
