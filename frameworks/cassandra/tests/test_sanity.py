...
def test_config_update_across_restart():
    ...
    for _ in range(3):
        ...
        sdk_plan.wait_for_completed_recovery(foldered_service_name, timeout_seconds=config.DEFAULT_CASSANDRA_TIMEOUT)

        rc, stdout, stderr = sdk_cmd.service_task_exec(foldered_service_name, "node-0", "env")
        ...
        if not stdout[envvar_pos + len(envvar):].startswith("15"):
